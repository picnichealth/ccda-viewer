const hipeHostname = window.location.hostname
	.split('.')
	.slice(-2)
	.toSpliced(0, 0, 'hipe')
	.join('.');
...
$(document).ready(() => {
	$('.viewbtn').off('click').click(render);
	init();
	loadCda(getQueryParams().get('ccdId'));
});
...
async function loadCda(ccdId) {
	if (ccdId) await loadtextarea(`https://${hipeHostname}/api/v0/ccds/${ccdId}.xml?raw=true`);
}

async function loadtextarea(url) {
	...
	$('#cdaxml').val(await res.text());
	render();
}

function render() {
	const id_target = $('.viewbtn').attr('id_target');
